...
<table align="center" border="1" width="70%">
    ...
    %>
    	...
    	<tr>
    	   <td>Телефоны:</td>
    	   <td> <a href = "<%=request.getContextPath()%>/?action=addPhoneNumber&person_id=<%=person.getId() %>">Добавить</a></td>
          ...
          for(String phone : person.getPhones().values())
           {
         	out.write(phone + "\n");
        %> 
        <a href="<%=request.getContextPath()%>/?action=editPhoneNumber&person_id=<%=person.getId()%>&phone_id=<%=person.getPhones().values()%>">Редактировать</a>
         <a href="<%=request.getContextPath()%>/?action=deletePhoneNumber&person_id=<%=person.getId()%>&phone_id=<%=person.getPhones().values()%>">Удалить</a>
    	<br/>
    <%
    } 
    %>
    
    <a href="<%=request.getContextPath()%>/?action=addPhoneNumber&person_id=<%=person.getId()%>">Добавить</a>        		
	</td>
	</tr>
	<%
    }  
